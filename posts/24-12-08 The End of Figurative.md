...
Much in the way Matías bumped up against limitations, I too often run into things that can't be done simply due to the nature of SwiftUI. And I don't even blame Apple for that. Things like how I can't let someone design in native SwiftUI on their Mac without the DetailsPro app being a Catalyst app. We all know Catalyst isn't going to be around forever, so what I am to do? One day, suddenly the DetailsPro app on Mac displays everything with Mac-based magical UI values, instead of the iOS ones that have been available to my users all this time?
